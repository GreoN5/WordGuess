...
namespace WordGuess
{
	public class Game
	{
		...
		private string _wordInProgress;
			...
			else
			{
				while (this._numberOfTries > 0)
				{
					...
					else
					{
						...
						this._wordInProgress = WordInProgress(this._randomWord.ToCharArray(), letter, builder);
						...
						{
							if (CheckForContainingLetters(this._wordInProgress.ToCharArray(), letter))
							{
								numberOfLetters--;
								ShowProgressOfWord(this._wordInProgress);
							}
							else
							{
								this._numberOfTries--;
